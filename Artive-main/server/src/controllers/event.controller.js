...
const allEvents = asyncHandler(async (req, res) => {
	...
	const response = { events, totalEvents };
	if (pagination) {
	...
	const pipeline = [
		...
		},
		{
			$addFields: {
				...
				// FIX: was "$eventLikes.userId" — the Likes schema field is "user" not "userId"
				// OLD: "$eventLikes.userId"  → isLiked was ALWAYS false for every user
				isLiked: {
					$in: [
						new mongoose.Types.ObjectId(userId),
						"$eventLikes.user", // FIX: correct field name from Likes model
					],
				},
			},
		},
...
const createEvent = asyncHandler(async (req, res) => {
	...
	let counter = 1;
	while (slugExists) {
		slug = `${slugify(name, { lower: true, strict: true })}-${Date.now()}-${counter}`;
...
const deleteEvent = asyncHandler(async (req, res) => {
	...
	// FIX: The pre('deleteOne') hook on Event model uses {document:true, query:false}
	// so it DOES NOT fire for findByIdAndDelete (query-level).
	// We must manually delete associated Likes here.
	await Likes.deleteMany({ event: deletedEvent._id });

	// Delete event photos from Cloudinary
	// FIX: use extractPublicId() which correctly handles asset_folder prefix
	// OLD: photoUrl.split("/").slice(-1)[0].split(".")[0]
	//   → stripped folder path → destroy() always got "not found" silently
	await Promise.all(
		deletedEvent.photos.map((photoUrl) => {
...
// FIX: $match should come BEFORE $sort for index efficiency
// OLD pipeline: [$sort, $match] → sorted entire collection before filtering
// FIXED pipeline: [$match, $sort]
const searchEvent = asyncHandler(async (req, res) => {
	...
	// FIX: $match BEFORE $sort
	const basePipeline = [
		...(matchStage ? [matchStage] : []),
		{ $sort: { createdAt: -1, _id: -1 } },
	];

	const totalResult = await Event.aggregate([...basePipeline, { $count: "total" }]);
